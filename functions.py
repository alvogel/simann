...
###### algorithm ######
def calc_cost(path_tmp,
              distance_lookup_table):  # berechnet die "Energie" der Konfiguration path_tmp mithilfe des distance_lookup_table
...
def metropolis(delta_energy,
               temperature):  # gibt zurück, ob die Konfiguration beibehalten wird (True) oder nicht (False) und benutzt dabei den Metropolis Algorithmus
    ...
    if delta_energy < 0:  # gibt True zurück, wenn die Energie der neuen Konfiguration kleiner ist als die der alten Konfiguration
        ...
        if uni_rand < math.exp(-(
                delta_energy / temperature)):  # gibt True mit einer Wkeit math.exp(-(delta_energy / temperature)) zurück
            return True
        else:
            return False
...
###### flip-types ######
def swap_row(
        old_path):  # erstellt neuen Weg, indem die Position von zwei zufällig ausgewählten aufeinanderfolgenden Städten in der Liste getauscht wird
...
def swap_random(
        old_path):  # erstellt neuen Weg, indem die Position von zwei zufällig ausgewählten Städten in der Liste getauscht werden
...
def revert_part(
        old_path):  # erstellt neuen Weg, indem ein Stück des Weges zwischen zwei zufällig ausgewählten Städten umgedreht wird
    rand_pos1 = random.randint(0, len(old_path) - 1)
    rand_pos2 = random.randint(0, len(old_path) - 1)
